...
class BaseRecLabelDecode(object):
    ...
    def __init__(self,
        ...
        if character_type == "en":
            ...
            if use_space_char: self.character_str += " "
            dict_character = list(self.character_str)
        else:
    ...
    def decode(self, text_index, text_prob=None, is_remove_duplicate=False):
        ...
        for batch_idx in range(batch_size):
            ...
            for idx in range(len(text_index[batch_idx])):
                if text_index[batch_idx][idx] in ignored_tokens: continue
                if is_remove_duplicate:
                    # only for predict
                    if idx > 0 and text_index[batch_idx][idx - 1] == text_index[batch_idx][idx]: continue
...
class CTCLabelDecode(BaseRecLabelDecode):
    ...
    def __call__(self, preds, label=None, *args, **kwargs):
        if isinstance(preds, paddle.Tensor): preds = preds.numpy()
        preds_idx = preds.argmax(axis=2)
        preds_prob = preds.max(axis=2)
        text = self.decode(preds_idx, preds_prob, is_remove_duplicate=True)
        if label is None: return text, preds_idx, preds_prob
        label = self.decode(label)
        return text, label, preds_prob
