...
import numpy as np
from configs.config import train_config_path
...
def train(config,
    ...
    for epoch in range(start_epoch, epoch_num + 1):
        ...
        for idx, batch in enumerate(train_dataloader):
            ...
            if idx >= len(train_dataloader): break
            lr = optimizer.get_lr()
            images = batch[0]
            if use_srn:
                others = batch[-4:]
            ...
            if dist.get_rank() == 0 and global_step > 0 and global_step % print_batch_step == 0:
                ...
                strs = 'epoch: [{}/{}], iter: {}, {}, reader_cost: {:.5f} s, batch_cost: {:.5f} s, samples: {}, ips: {:.5f}'.format(epoch, epoch_num, global_step, logs, train_reader_cost / print_batch_step, train_batch_cost / print_batch_step, batch_sum, batch_sum / train_batch_cost)
                logger.info(strs)
                train_batch_cost = 0.0
                train_reader_cost = 0.0
                batch_sum = 0
...
def preprocess(is_train=False):
    ...
    assert alg in ['EAST', 'DB', 'SAST', 'Rosetta', 'CRNN', 'STARNet', 'RARE', 'SRN', 'CLS']
